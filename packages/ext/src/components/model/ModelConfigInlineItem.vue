...
<template>
  ...
  <div class="flex items-center gap-1">
    <img v-if="modelProviderPresets[item.providerType].icon" :src="modelProviderPresets[item.providerType].icon"
      alt="Model Icon" class="w-4 h-4" />


    <div class="flex flex-col text-xs max-w-32 ">
      <div class="overflow-x-hidden text-nowrap text-ellipsis" :title="item.modelName">
        {{ item.name }}
      </div>
      <div v-if="!isSelected" class="overflow-x-hidden text-nowrap text-ellipsis" :title="item.modelName">
        {{ item.modelName }}
      </div>
